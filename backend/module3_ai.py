...
def reload_summarizer(model_path: str):
    global SUMMARIZER
    if not ML_AVAILABLE:
        logger.warning(" ▲ ML stack missing — cannot reload summarizer.")
        return
    try:
        SUMMARIZER = pipeline("summarization", model=model_path, device=0 if torch.cuda.is_available() else -1)
        logger.info(" 🟡️ Summarizer reloaded from %s", model_path)
    except Exception as e:
        logger.warning(" ▲ Failed to reload summarizer: %s", e)